    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-24 md:pl-64">
      ...
      <div className="hidden md:flex px-6 py-5 items-center justify-between sticky top-0 bg-[#F8FAFC]/80 backdrop-blur-md z-10 border-b border-slate-100">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-base font-bold text-slate-900">AI Health Insights</h1>
        </div>
        <div className="w-10"></div>
      </div>

      <motion.div 
        className="px-6 py-6 max-w-[1200px] mx-auto space-y-6"
      ...
      >
        ...
        <motion.div variants={itemVariants}>
          <h2 className="text-sm font-bold text-slate-900 mb-3 ml-1">AI-Generated Recommendations</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {data.aiRecommendations?.map(rec => (
              <RecommendationCard key={rec.id} recommendation={rec} />
            ))}
            {(!data.aiRecommendations || data.aiRecommendations.length === 0) && (
              <div className="col-span-3 text-center py-8 text-sm text-slate-500 font-medium">No insights available at the moment.</div>
            )}
          </div>
        </motion.div>
          ...
          <motion.div variants={itemVariants} className="h-[380px]">
            <Card className="bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] border-0 rounded-3xl h-full flex flex-col">
              <CardContent className="p-6 flex-1 flex flex-col">
                <h3 className="text-sm font-bold text-slate-900 mb-6">Risk Level Distribution</h3>
                ...
                <div className="flex-1 overflow-y-auto scrollbar-hide p-2">
                  {data.highRiskPatients?.map(patient => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-slate-500 truncate">{patient.age} yrs • {patient.village || 'Unknown Village'}</p>
                      </div>
                      <div className="shrink-0 flex items-center gap-3">
                        <span className="text-[10px] font-bold text-red-600 bg-red-50 px-2 py-1 rounded-md">Critical</span>
                  ...
                  {(!data.highRiskPatients || data.highRiskPatients.length === 0) && (
                    <div className="h-full flex flex-col items-center justify-center text-center p-6">
                      ...
                      <p className="text-xs text-slate-500 mt-1">Excellent! No critical interventions needed right now.</p>
                    </div>
                  )}
                </div>
              </CardContent>
    ...
    <Card className={`bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-3xl overflow-hidden hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all border ${border} relative group cursor-pointer`}>
      <CardContent className="p-5 flex flex-col h-full relative z-10">
        ...
        <div className="mt-4 flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity">
          <span className={`text-[10px] font-bold ${color}`}>Take Action</span>
